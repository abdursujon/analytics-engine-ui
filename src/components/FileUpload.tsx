...
function detectPastedFormat(text: string): {
  ext: string;
  mime: string;
} {
  ...
  if (
    ...
    return { ext: "ndjson", mime: "application/x-ndjson" };
  }
...
export default function FileUpload() {
  ...
  function acceptFile(f: File | null | undefined) {
    ...
    if (
      ...
      setError("Allowed formats are: parquet, json, ndjson, csv, txt");
      setFile(null);
      setFileName(null);
      setAnalysis(null);
      return;
  ...
  function handlePasteChange(e: ChangeEvent<HTMLTextAreaElement>) {
    ...
    const { ext, mime } = detectPastedFormat(text);
    const name = `sample.${ext}`;
    acceptFile(new File([text], name, { type: mime }));
  }
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (name.endsWith(".parquet")) {
        ...
        res = await fetch(`${API_BASE_URL}/analytics-engine/ingestParquet`, {
          method: "POST",
          body: formData,
        });
      } else if (name.endsWith(".ndjson")) {
        res = await fetch(`${API_BASE_URL}/analytics-engine/ingestNdjson`, {
          method: "POST",
          headers: { "Content-Type": "application/x-ndjson" },
          body: file,
        });
      } else if (name.endsWith(".json")) {
        res = await fetch(`${API_BASE_URL}/analytics-engine/ingestJson`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: file,
        });
      } else {
        const contentType = name.endsWith(".csv") ? "text/csv" : "text/plain";
        res = await fetch(`${API_BASE_URL}/analytics-engine/ingestCsv`, {
          method: "POST",
      ...
      if (!res.ok) {
        const frontendHint =
          file.size === 0 ? "The file is empty." : "The file content is invalid.";
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok && res.status !== 204) {
        setError(
          res.status === 404 ? "Analysis no longer exists." : "Delete failed",
        );
        return;
      }
  ...
  const detected = pasteText.trim() ? detectPastedFormat(pasteText) : null;
        ...
        <div className="rounded-2xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-[1px] shadow-xl shadow-indigo-500/10">
          <div className="rounded-2xl bg-white/95 p-6 backdrop-blur-sm">
            ...
            >
              <input
                type="file"
                accept=".parquet, .csv, .json, .ndjson, .txt"
              ...
              {file && !pasteText ? (
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="text-xs text-slate-500">
                    PARQUET, JSON, NDJSON, CSV or TXT up to 5&nbsp;MB
                  </div>
                </div>
              )}
            </label>
            ...
            <div className="relative">
              <textarea
                ...
                spellCheck={false}
                placeholder="Paste CSV, JSON, NDJSON or TXT content here…"
                className="block h-40 w-full resize-y rounded-xl border border-slate-200 bg-slate-50/60 p-3 font-mono text-xs leading-relaxed text-slate-800 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-200"
              />
              {pasteText && (
                <button
                  type="button"
                  onClick={clearPaste}
                  className="absolute right-2 top-2 rounded-md bg-white/90 px-2 py-1 text-[11px] font-medium text-slate-500 shadow-sm transition hover:cursor-pointer hover:text-slate-800"
                >
                  Clear
                </button>
              )}
            ...
            {detected && (
              <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <span>Detected:</span>
                <span className="rounded-full bg-indigo-100 px-2 py-0.5 font-mono text-[11px] font-medium text-indigo-700">
                  sample.{detected.ext}
                </span>
          ...
          {view === "summary" ? (
            <>
              <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <Stat
                  label="Rows"
                  value={analysis.numberOfRows?.toLocaleString() ?? "—"}
                />
                <Stat
                  label="Columns"
                  value={analysis.numberOfColumns?.toLocaleString() ?? "—"}
                />
                <Stat
                  label="Characters"
                  value={analysis.totalCharacters?.toLocaleString() ?? "—"}
                />
                <Stat
                  label="Numeric cols"
                  value={String(
                    analysis.columnStatistics?.filter((c: any) => c.isNumeric)
                      .length ?? 0,
                  )}
                />
              </div>

              {Array.isArray(analysis.columnStatistics) &&
                analysis.columnStatistics.length > 0 && (
